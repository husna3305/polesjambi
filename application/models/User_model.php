...
class User_model extends CI_Model
{
  ...
  {
    $where = 'WHERE 1=1';
    $select = 'mu.id_user,mu.id_group,email,username,nama_lengkap,no_hp,img_small,img_big,mu.created,mu.aktif,mu.created_by,mu.updated_at,mu.updated_by,nama_group,mu.last_login';
    if ($filter != null) {
    ...
    if (isset($filter['order'])) {
      $order = $filter['order'];
      $order_column = [null, 'username', 'nama_lengkap', 'email', 'no_hp', 'nama_group', 'last_login', 'mu.aktif', null];
      if ($order != '') {
