...
class Auth extends Crm_Controller
{
	...
	public function login()
	{
		...
		if (isset($_POST['email'])) {
			//Cek Captcha
			$cek_c = $this->_validate_captcha($this->input->post('g-recaptcha-response'));
			if ($cek_c == false) {
				$response = ['status' => 0, 'pesan' => 'Silahkan validasi captcha terlebih dahulu'];
				send_json($response);
			}
...
<?php
	...
	function _validate_captcha($response)
	{
		$captcha_response = trim($response);

		if ($captcha_response != '') {
			$keySecret = '6LflfkUcAAAAAA0CBHdxyDj87ZyPes-B5KGFWdrU';
